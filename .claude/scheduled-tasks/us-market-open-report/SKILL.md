...
>
> ⚠️ **禁止在段落文字（非表格）內使用 `$數字` 格式**：MDX parser 會把兩個 `$` 之間的內容解讀為 LaTeX math 表達式，導致版面錯亂。段落中的金額與價格請直接寫數字（例：「開盤來到 441」），或補上單位（「101.50 美元」）。`$` 符號只允許出現在表格欄位內。
...
生成 MDX 內文後，寫入檔案前先掃描以下問題：

| 檢查項目 | 規則 | 修正方式 |
|---|---|---|
| 段落 `$數字` 配對 | 段落（非表格）中每出現一個 `$數字`，往後掃描是否有第二個 `$`，配對即觸發 math 解析 | 移除 `$`，直接寫數字 |
| 全形括號內粗體 | `（**文字**）` 形式 | 移除粗體，改為純文字 |
| 表格欄位數一致 | 每列欄位數需與 header 相同 | 補齊或刪除多餘欄位 |
| blockquote 緊貼段落 | `>` 前需有空行 | 補空行 |

若發現問題，先在記憶體中修正，再進行步驟 4。
